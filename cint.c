...
static void cint_random_bits(cint *num, size_t bits, uint64_t * seed) {
	...
	cint_erase(num);
	uint64_t *r = *seed ? seed : (*seed = 0x4eec588af4f90e1, seed);
...
int cint_is_prime(cint_sheet *sheet, const cint *N, int iterations, uint64_t * seed) {
	...
	if (0 < N->nat && *N->mem < 961 && N->mem + 1 >= N->end) {
		...
		if (iterations <= 0)
			...
			iterations = bits >= 1300 ? 3 : bits >= 850 ? 4 : bits >= 650 ? 5 : bits >= 550 ? 6 : bits >= 450 ? 7 : bits >= 400 ? 8 : bits >= 350 ? 9 : bits >= 300 ? 10 : bits >= 250 ? 13 : bits >= 200 ? 16 : bits >= 150 ? 18 : 24 ;
		cint_dup(A, N);
