...
int cint_equals(const cint *lhs, const cint *rhs) {
	...
	if ((lhs->end - lhs->mem) != (rhs->end - rhs->mem))
		return 0 != printf("[ERROR] The lengths of LHS and RHS are different: %lld vs %lld\n", lhs->end - lhs->mem, rhs->end - rhs->mem);

	// The numbers are equals in terms of sign.
	if (lhs->nat != rhs->nat)
		return 0 != printf("[ERROR] The signs of the numbers are different: %lld vs %lld\n", lhs->nat, rhs->nat);
	...
	for (int bits = 1; bits <= 150; bits++) {
		for (int sign = 1; sign >= -1; sign -= 2) {
			...
			if (strlen(buf) != expected_length)
				return 0 != printf("[ERROR] The bit count for %d bits isn't correct: want %zu and found %llu\n", bits, expected_length, strlen(buf));
	...
	for (int bits_A = 0; bits_A <= 200; bits_A++) {
		for (int bits_B = 1; bits_B <= 200; bits_B++) {
			...
			if (R->mem != R->end && R->nat != A->nat)
				return 0 != printf("[ERROR] The convention is that the sign of R (%lld) is the sign of A (%lld)\n", R->nat, A->nat);
...
void cint_factorial(cint_sheet *sheet, long long int factorial_n, cint *res) {
	cint *B = h_cint_tmp(sheet, 0, res);
	cint *D = h_cint_tmp(sheet, 1, res);
	cint_reinit(res, 1);
	for (long long int i = 0; i < factorial_n;) {
		cint_reinit(D, ++i);
		cint_mul(res, D, B);
		cint_reinit(D, ++i);
		cint_mul(B, D, res);
...
void print_factorial(const int factorial_n) {
	int bits = 2 ; // Print the factorial.
	for(int i = 1; i < factorial_n; ++i, ++bits)
		for(int j = i; j >>= 1; ++bits);
